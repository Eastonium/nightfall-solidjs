...
interface ButtonProps extends JSX.ButtonHTMLAttributes<HTMLButtonElement>, ButtonWrapperProps {
	bold?: boolean;
...
const NormalButton: Component<ButtonProps> = props => {
	const [p, buttonProps] = splitProps(
		mergeProps({ fill: false, bold: false, color: "blue", big: false, wrapperProps: {} }, props),
		["color", "fill", "wrapperProps", "children"],
	);

	return (
		<ButtonWrapper fill={p.fill} {...p.wrapperProps}>
			<NormalStyledButton color={p.color} {...buttonProps}>
	...
	&:enabled {
		...
		&:focus {
			> span {
				background: linear-gradient(
					to bottom right,
					${p => getGradientColors(buttonColors[p.color], true).join(", ")}
				);
			}
		}
	}
...
export const MetalButton: Component<MetalButtonProps> = props => {
	const [p, buttonProps] = splitProps(
		mergeProps({ fill: false, dark: false, small: false, wrapperProps: {} }, props),
		["fill", "wrapperProps"],
	);
	return (
		<ButtonWrapper fill={p.fill} {...p.wrapperProps}>
...
const MetalStyledButton = styled("button")<{ dark: boolean; small: boolean }>`
	...
	border-style: solid;
	border-image: linear-gradient(${p => (p.dark ? "#68719C , #31384E" : "#EEE, #999")}) 1;
	${p => (p.dark ? css({ color: "#E0E2EB" }) : css({ textShadow: "0px 1px #FFFB" }))}
	background: linear-gradient(${p => (p.dark ? "#52597D, #282E40" : "#DDD, #777")});
	...
	&::after {
		...
		border-style: solid;
		border-image: linear-gradient(${p => (p.dark ? "#444B69, #212635" : "#BBB, #555")}) 1;
	}
	...
	&:enabled {
		...
		&:focus {
			border-image: linear-gradient(
					${p => (p.dark ? "#334ED1 , #14286C" : "#FFF, #CDE1FE, #669EFF")}
				)
				1;
			${p => (p.dark ? css({ color: "#E0E2EB" }) : css({ textShadow: "0px 1px #FFFB" }))}
			...
			&::after {
				border-image: linear-gradient(
						${p => (p.dark ? "#20348D, #0D1C49" : "#7BEAEA, #1E76FA, #0054E6")}
					)
					1;
			}
		}
